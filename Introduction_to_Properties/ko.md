...
-   **반올림**: 나누기를 한 뒤 소수점을 처리하는 *명령*
    -   **Floor**: the floor() 명령은 소수값을 *버린다*. **예제**: `HitPoints:{floor(Constitution / 2)}` 이 명령은 Constitution를 2로 나눈뒤 소수값을 버린다.
    -   **Ceiling**: the ceil() 명령은 floor()와 비슷하지만 소수값을 *올린다*. **예제**: `HitPoints:{ceil(Constitution/2)}`
